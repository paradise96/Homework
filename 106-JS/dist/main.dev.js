...
document.body.prepend(orderedList); //Створити HTML-сторінку з кнопкою "Відкрити" і модальним вікном. На модальном вікні повинен бути текст і кнопка "Закрити". Спочатку модальне вікно не відображається. При кліку на кнопку "Відкрити" з'являється модальне вікно, на кнопку "Закрити" — зникає.
...
divBlock.prepend(text); // const closeBtn = document.getElementById('close-btn');
...
var modalWindow = document.createElement('h2');
...
function toggle() {
  ...
  showBody.classList.toggle('active'); // if(showBody.classList.contains('active')){
  //     console.log('hello');
...
switcher.innerText = 'Switch'; // Create wrapper and append it

var wrap = document.createElement('div');
wrap.style.display = 'flex';
wrap.style.flexDirection = 'column';
wrap.style.gap = '20px';
trafficLight.append(wrap); //create three divs with different ids and append it inside wrapper
